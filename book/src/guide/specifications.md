...
Sometimes Flux cannot check `#[derive(..)]` generated code (e.g. `Eq` instances for `enum`
which use `discriminant`). For such cases use `#[flux_rs::trusted_derive]` to tell
...
- **Opaque types are skipped.** A `#[flux_rs::opaque]` type implicitly trusts its derives,
  since derived code necessarily reads the representation that opacity forbids. You do not need to write `#[flux_rs::trusted_derive]` on an opaque type.
...
An explicit annotation supersedes implicit behavior, meaning, an opaque type can opt its
