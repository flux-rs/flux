...
Code generated by a `#[derive(..)]` on an opaque struct hits this same restriction, but cannot
be annotated by hand. Flux handles that case for you; see [derive-generated
code](#derive-generated-code).
...
Flux checks the code generated by a `#[derive(..)]` just like any other code. Sometimes that
code is something Flux cannot check — most commonly a derive on an [opaque
struct](#opaque-structs), where the generated `Debug` or `Hash` reads the very fields that
opacity hides. Marking it `#[flux_rs::trusted]` is not an option, because you cannot attach an
...
- **Opaque types get this for free.** A `#[flux_rs::opaque]` type implicitly trusts its derives,
  since derived code necessarily reads the representation that opacity forbids, so it could
  never be checked. You do not need to write `#[flux_rs::trusted_derive]` on an opaque type.
- **The attribute is not recursive.** Unlike `#[flux_rs::ignore]` and `#[flux_rs::trusted]`,
  which apply to all nested items, `#[flux_rs::trusted_derive]` applies only to the type it is
  written on. Putting it on a module does nothing for the types inside.

An explicit annotation always wins over the implicit behavior, so an opaque type can opt its
derives back in to checking with `#[flux_rs::trusted_derive(no)]`. The attribute also accepts a
reason, as in `#[flux_rs::trusted_derive(reason = "...")]`.

Note that the annotation is all-or-nothing: it trusts everything in the `derive` list, not
individual traits.

```rust,noplayground
{{#include ../../../tests/tests/pos/surface/trusted_derive00.rs}}
```
...
Sometimes you may want to write specs for functions, structs, enums
