...
use crate::{ConstDecl, ThyFunc, Types, graph::FxIndexSet};
...
impl<T: Types> Expr<T> {
        ...
        T::Var: Eq + Hash,
    {
        match self {
            ...
            Expr::Var(v) => {
                if let Some(replacement) = subst_map.get(v) {
                    replacement.clone()
                } else {
                    self.clone()
                }
            ...
            Expr::Exists(bound_vars, expr) => {
                ...
                let mut shadowed = subst_map.clone();
        ...
        match self {
            Expr::App(head, sort_args, args, out_sort) => {
                ...
                match uncurried_head {
                    Expr::App(head_head, head_sort_args, mut head_args, head_out) => {
                        ...
                        Expr::App(head_head, new_sort_args, head_args, head_out.or(out_sort.clone()))
                    }
            ...
            Expr::Exists(binders, expr) => {
                Expr::Exists(binders.clone(), Box::new(expr.uncurry()))
            }
            Expr::WKVar(WKVar { wkvid, args }) => {
                Expr::WKVar(WKVar {
                    wkvid: wkvid.clone(),
    ...
    pub fn strip_wkvars(&self) -> Self {
        match self {
            ...
            Expr::Exists(binders, expr) => Expr::Exists(binders.clone(), Box::new(expr.strip_wkvars())),
        }
    }
