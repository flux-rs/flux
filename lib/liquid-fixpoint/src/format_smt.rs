...
fn fmt_expr_smt<T: Types>(expr: &Expr<T>, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match expr {
        ...
        Expr::Exists(sorts, body) => {
            // Our Exists has anonymous sorts (bound variables)
            write!(f, "(exists (")?;
        ...
        // BoundVar should not appear in ground formulas sent to hornspec
        Expr::BoundVar(_) => write!(f, "_bv_"),
    }
}
...
fn fmt_fun_def<T: Types>(fun: &FunDef<T>, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    for (i, (name, sort)) in fun.args.iter().enumerate() {
        if i > 0 {
            write!(f, " ")?;
        }
    ...
    write!(f, ") ")?;
    fmt_sort_smt(&fun.out, f)?;
    write!(f, " ")?;
    fmt_expr_smt(&fun.body, f)?;
    writeln!(f, ")")
}
