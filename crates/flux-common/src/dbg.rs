...
pub fn dump_multi_item_info_for<T: fmt::Debug>(
    tcx: TyCtxt,
    def_id: DefId,
    ext: impl AsRef<str>,
    val: T,
    ...
    let path =
        config::log_dir().join(format!("{crate_name}-multi-check-{item_name}.{}", ext.as_ref()));
    let file = fs::File::create(path)?;
    let mut writer = std::io::BufWriter::new(file);
    writeln!(writer, "{val:#?}")
}
...
macro_rules! _call{
    ($genv:expr, $callee_def_id:expr, $caller_def_id:expr, $span:expr) => {{
        ...
        let callee_spec = genv
            ...
            .unwrap_or_default();
        let span_json = SpanTrace::new(tcx, $span);
        ...
        tracing::warn!(
            ...
            src_span = ?span_json,
            dst_span = ?callee_span_json,
        )
    }};
}
pub use crate::_call as call;
