...
fn check_crate(genv: GlobalEnv) -> Result<(), ErrorGuaranteed> {
    tracing::info_span!("check_crate").in_scope(move || {
        ...
        let lean_result: Result<(), ErrorGuaranteed> = Ok(());
    ...
    fn check_def(&mut self, def_id: LocalDefId) -> Result<(), ErrorGuaranteed> {
        ...
        match kind {
            DefKind::Fn | DefKind::AssocFn => {
                ...
                if is_fn_with_body {
                    refineck::check_fn(genv, &mut self.cache, &mut self.constraints, local_id)?;
                }
            }
            DefKind::Enum => {
            ...
            DefKind::Static { .. } => {
                ...
                {
                    refineck::check_static(genv, &mut self.cache, &mut self.constraints, local_id, ty)?;
                }
            }
            _ => (),
        }
