...
pub struct Specs {
    ...
    pub ignores: UnordMap<LocalDefId, surface::Ignored>,
    pub trusted: UnordMap<LocalDefId, surface::Trusted>,
    pub trusted_impl: UnordMap<LocalDefId, surface::Trusted>,
    pub infer_opts: UnordMap<LocalDefId, config::PartialInferOpts>,
    pub should_fail: UnordSet<LocalDefId>,
    /// Set of functions annotated with `#[proven_externally]`
    pub proven_externally: UnordSet<LocalDefId>,
...
impl Specs {
    ...
    pub fn finish(&mut self, tcx: TyCtxt) {
        let mut f = |def_id: LocalDefId, attrs: &[surface::Attr]| {
            for attr in attrs {
                match attr {
                    surface::Attr::Trusted(trusted) => {
                        self.trusted.insert(def_id, *trusted);
                    }
                    surface::Attr::TrustedImpl(trusted) => {
                        self.trusted_impl.insert(def_id, *trusted);
                    }
                    surface::Attr::Ignore(ignored) => {
                        self.ignores.insert(def_id, *ignored);
                    }
                    surface::Attr::ProvenExternally => {
                        self.proven_externally.insert(def_id);
                    }
                    surface::Attr::InferOpts(opts) => {
                        self.infer_opts.insert(def_id, *opts);
                    }
