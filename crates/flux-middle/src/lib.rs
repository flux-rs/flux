...
pub struct Specs {
    ...
    /// Maps function DefIds to the source text of their #[sig(...)] attribute
    spec_attr_strings: UnordMap<DefId, String>,
...
impl Specs {
    ...
    pub fn set_spec_attr_span(&mut self, def_id: DefId, span: Span) {
        self.spec_attr_spans.insert(def_id, span);
    }

    pub fn get_spec_attr_span(&self, def_id: DefId) -> Option<Span> {
        self.spec_attr_spans.get(&def_id).copied()
    }

    pub fn set_spec_attr_string(&mut self, def_id: DefId, s: String) {
        self.spec_attr_strings.insert(def_id, s);
    }

    pub fn get_spec_attr_string(&self, def_id: DefId) -> Option<&str> {
        self.spec_attr_strings.get(&def_id).map(|s| s.as_str())
    }
