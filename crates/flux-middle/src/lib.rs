...
pub struct Specs {
    ...
    /// Set of dummy items generated by the extern spec macro we must completely ignore. If an item
    /// is in this sets, all its descendants are also consider dummy (they may not be in the set).
...
impl MaybeExternId<LocalDefId> {
    pub fn as_resolved_def_id(self) -> DefId {
        match self {
