...
impl<'genv, 'tcx> GlobalEnv<'genv, 'tcx> {
    ...
    ///
    /// A `#[flux::opaque]` type implicitly trusts its derives: the generated code reads the
    /// internal representation, which is exactly what opacity forbids, so it could never be
    /// checked. An explicit `#[flux::trusted_derive(..)]` takes precedence, which lets an opaque
    ...
    /// self type of the enclosing impl. Returns `None` if the self type is not a local ADT.
