...
impl RefineTree {
    ...
    pub(crate) fn wkvars_in_positions(
        &self,
    ) -> (FxIndexSet<WKVid>, FxIndexSet<WKVid>, FxIndexMap<WKVid, usize>) {
        struct WKVars {
            heads: FxIndexSet<WKVid>,
            assumptions: FxIndexSet<WKVid>,
            self_args: FxIndexMap<WKVid, usize>,
        }

        fn visit_expr(
            expr: &Expr,
            vars: &mut FxIndexSet<WKVid>,
            self_args: &mut FxIndexMap<WKVid, usize>,
        ) {
            struct Visitor<'a> {
                vars: &'a mut FxIndexSet<WKVid>,
            }
