...
use flux_middle::{
    def_id::{MaybeExternId, ResolvedDefId}, global_env::GlobalEnv, pretty, rty::{
        self, fold::{TypeFolder, TypeVisitable}, ESpan, EarlyBinder, Name
    }, timings, FixpointQueryKind
};
...
fn add_fn_fix_diagnostic<'a>(
    ...
    let fixed_fn_sig_snippet = format!("{:?}", pretty::with_cx!(&pretty::PrettyCx::default(genv).hide_regions(true), &solved_fn_sig));
