...
use flux_infer::{
    fixpoint_encoding::{
        FixQueryCache, FixpointCheckError, KVarGen, PossibleSolutions, SolutionTrace, TagIdx,
    },
...
fn collect_body(
    ...
    let (tree, kvars) = infer_body(genv, def_id, poly_sig)?.split_for_multi();
...
pub fn check_fn(
    ...
    cache: &mut FixQueryCache,
    def_id: LocalDefId,
    multi_trees: &mut Vec<(MaybeExternId, RefineTree, KVarGen)>,
) -> Result<(), ErrorGuaranteed> {
    let span = genv.tcx().def_span(def_id);
    let selected = config::multi_check_pattern()
        .is_some_and(|_| genv.multi_check(MaybeExternId::Local(def_id)));
    ...
    metrics::time_it(TimingKind::CheckBody(def_id), || -> Result<(), ErrorGuaranteed> {
        ...
        if selected {
            multi_trees.push(collect_body(genv, def_id, &poly_sig)?);
            Ok(())
        } else {
