...
use crate::{
    FluxMetadata,
    cargo_flux_opts::CargoFluxCommand,
    utils::{
        EXIT_ERR, flux_sysroot_dir, get_binary_path, get_flux_driver_path, get_rust_toolchain,
    },
};

pub fn run(cargo_flux_cmd: CargoFluxCommand) -> anyhow::Result<i32> {
...
fn env_flags(name: &str) -> Vec<String> {
    env::var(name)
        .map(|flags| parse_flux_flags(&flags))
        .unwrap_or_default()
}

fn parse_flux_flags(flags: &str) -> Vec<String> {
    flags.split(" ").map(Into::into).collect()
}
...
mod tests {
    ...
    #[test]
    fn global_flags_preserve_existing_space_separated_parsing() {
        assert_eq!(parse_flux_flags("-Fcache=off -Ftimings"), ["-Fcache=off", "-Ftimings"]);
    }
