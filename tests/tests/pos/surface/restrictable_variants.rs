...
enum Expr {
    ...
    Cst(bool),
    #[variant((Box<Expr[@s]>) -> Expr[set_union(s, #{ExprLbl::Not})])]
    Not(Box<Expr>),
    #[variant((Box<Expr[@s1]>, Box<Expr[@s2]>) -> Expr[set_union(set_union(s1, s2), #{ExprLbl::Or})])]
    Or(Box<Expr>, Box<Expr>),
    #[variant((Box<Expr[@s1]>, Box<Expr[@s2]>) -> Expr[set_union(set_union(s1, s2), #{ExprLbl::And})])]
    And(Box<Expr>, Box<Expr>),
    #[variant((Box<Expr[@s1]>, Box<Expr[@s2]>) -> Expr[set_union(set_union(s1, s2), #{ExprLbl::Xor})])]
...
impl Expr {
    ...
    #[spec(
        fn(&Expr[@s])
        -> Expr { v: set_subset(v, set_union(set_difference(s, #{ExprLbl::Xor}), #{ExprLbl::And, ExprLbl::Or, ExprLbl::Not})) }
    )]
    ...
    #[spec(
        fn(&Expr[@s], _)
        -> Box<Expr{v: set_subset(v, set_union(set_difference(s, #{ExprLbl::Var}), #{ExprLbl::Cst}))}>
    )]
