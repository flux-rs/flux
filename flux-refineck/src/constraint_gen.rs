...
impl<'a, 'tcx> ConstrGen<'a, 'tcx> {
    ...
    pub(crate) fn check_fn_call(
        ...
        .map(|(actual, formal)| {
            if let (Ref!(Mutability::Mut, _), Ref!(Mutability::Mut, ty)) = (actual.kind(), formal.kind())
        ...
        for (actual, formal) in iter::zip(&actuals, inst_fn_sig.args()) {
            ...
            match (actual.kind(), formal.kind()) {
                ...
                (TyKind::Ptr(PtrKind::Mut, path), Ref!(Mutability::Mut, bound)) => {
                ...
                (TyKind::Ptr(PtrKind::Shr, path), Ref!(Mutability::Not, bound)) => {
                    let checker_config = infcx.checker_config;
...
impl<'a, 'tcx> InferCtxt<'a, 'tcx> {
    ...
    fn bty_subtyping(&mut self, rcx: &mut RefineCtxt, bty1: &BaseTy, bty2: &BaseTy) {
        match (bty1, bty2) {
            ...
            (BaseTy::Ref(Mutability::Mut, ty1), BaseTy::Ref(Mutability::Mut, ty2)) => {
                self.subtyping(rcx, ty1, ty2);
                self.subtyping(rcx, ty2, ty1);
            }
            (BaseTy::Ref(Mutability::Not, ty1), BaseTy::Ref(Mutability::Not, ty2)) => {
                self.subtyping(rcx, ty1, ty2);
            }
