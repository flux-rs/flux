...
impl<'genv, 'tcx> GlobalEnv<'genv, 'tcx> {
    ...
    /// `lookup_trait_impl(trait_f, substs)` finds the specific `DefId` that
    /// implements a trait-method `trait_f` for the given `substs` using
    /// rustc's trait resolution API.
    ///
    ...
    // ) -> Option<DefId> {
    //     match rustc_middle::ty::Instance::resolve(self.tcx, param_env, trait_f, substs.orig) {
    //         Ok(Some(instance)) => {
    ...
    // fn lookup_trait_impl_old(&self, trait_f: DefId, substs: &CallSubsts<'tcx>) -> Option<DefId> {
    //     // println!("TRACE: lookup_trait_impl trait_f = `{trait_f:?}` substs = `{:?}`", substs.orig);
    //     match substs.orig.get(0)?.unpack() {
    //         GenericArgKind::Type(self_ty) => {
    //             let trait_id = self.tcx.trait_of_item(trait_f)?;
    //             let impl_id = self
    //                 .tcx
    //                 .find_map_relevant_impl(trait_id, self_ty, |impl_id| {
    //                     let trait_ref = self.tcx.impl_trait_ref(impl_id)?;
    //                     if Self::matches_subst(&trait_ref.substs, &substs.orig) {
    //                         Some(impl_id)
    //                     } else {
    //                         None
    //                     }
    ...
    //             // println!("TRACE: lookup_trait_impl trait_f = `{trait_f:?}` self_ty = `{self_ty:?}` trait_id = `{trait_id:?}` impl_id = `{impl_id:?}` impl_f = `{impl_f:?}`");
    //             // println!("TRACE: {impl_f:?} ty = {:?}", self.tcx.fn_sig(impl_f));
    //             Some(*impl_f)
    //         }
    //         _ => None,
    //     }
    ...
    // fn matches_subst(trait_ref_subst: &SubstsRef<'tcx>, callsite_subst: &SubstsRef<'tcx>) -> bool {
    //     trait_ref_subst.get(0) == callsite_subst.get(0)
    // }
    ...
    // ) -> ty::PolySig {
    //     let trait_impl = self.lookup_trait_impl(def_id0, param_env, substs0);
    //     println!(
    //         "TRACE: lookup_trait_impl `{def_id0:?}` with substs `{:?}` is `{trait_impl:?}`",
    //         substs0.orig
    //     );
