...
pub trait TypeFoldable: Sized {
    ...
    ///
    /// [`holes`]: ExprKind::Hole
    fn replace_holes(&self, mk_pred: &mut impl FnMut(Sort) -> Binder<Expr>) -> Self {
        struct ReplaceHoles<'a, F>(&'a mut F, &'a Sort);

        impl<'a, F> TypeFolder for ReplaceHoles<'a, F>
        where
            F: FnMut(Sort) -> Binder<Expr>,
        {
            fn fold_binders<T: TypeFoldable>(&mut self, t: &Binder<T>) -> Binder<T> {
                t.super_fold_with(&mut ReplaceHoles(&mut *self.0, t.sort()))
            }

            fn fold_expr(&mut self, e: &Expr) -> Expr {
                if let ExprKind::Hole = e.kind() {
                    let binders = self.0(self.1.clone());
                    debug_assert_eq!(binders.sort(), self.1);
        ...
        self.fold_with(&mut ReplaceHoles(mk_pred, &Sort::unit()))
    }
