...
pub trait TypeFoldable: TypeVisitable {
    ...
    /// Replaces all Opaque-AliasTy holes with fresh-evars obtained from `mk_args` which takes
    ...
    fn replace_opaque_holes(&self, mk_args: impl FnMut(DefId) -> List<Expr>) -> Self {
            ...
            F: FnMut(DefId) -> List<Expr>,
        {
            ...
            fn fold_ty(&mut self, ty: &Ty) -> Ty {
                if let TyKind::Alias(AliasKind::Opaque, alias_ty) = ty.kind() {
                    debug_assert_eq!(self.1.len(), 0);
                    let refine_args = (self.0)(alias_ty.def_id);
                    let args = alias_ty.args.fold_with(self);
                    alias_ty.refine_args.iter().for_each(|e| {
                        debug_assert!(matches!(e.kind(), ExprKind::Hole));
                    });
                    debug_assert!(refine_args.len() == alias_ty.refine_args.len());
                    Ty::alias(
                        AliasKind::Opaque,
                        AliasTy { def_id: alias_ty.def_id, args, refine_args },
                    )
                } else {
                    ty.super_fold_with(self)
                }
        ...
        self.fold_with(&mut ReplaceOpaqueHoles(mk_args, vec![]))
    }

    /// Replaces all [holes] with a fresh predicate generated by calling `mk_pred`. The function
    /// `mk_pred` takes a list with all the *layers* of [bound] variables at the point the hole was
    /// found. Each layer corresponds to the list of sorts bound at that level. The list is ordered
    /// from outermost to innermost binder, i.e., the last element is the binder closest to the hole.
    ///
    /// [holes]: ExprKind::Hole
    /// [bound]: Binder
